...
type LogoItem = { src: string; srcSet?: string; sizes?: string; width?: number; height?: number; alt?: string; title?: string; href?: string }
type NodeItem = { node: ReactNode; href?: string; ariaLabel?: string; title?: string }
type LogoData = LogoItem | NodeItem
...
interface LogoLoopProps {
  ...
  renderItem?: (item: LogoData, key: string) => ReactNode
  ariaLabel?: string
  className?: string
  style?: CSSProperties
}
  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoData, key: string) => {
        ...
        const content = isNodeItem ? (
          <span
            className={cx(
              ...
              scaleOnHover &&
                'transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover/item:[transform:scale(1.2)]'
            )}
        ...
        const itemAriaLabel = isNodeItem ? (nodeItem.ariaLabel ?? nodeItem.title) : (logoItem.alt ?? logoItem.title);
