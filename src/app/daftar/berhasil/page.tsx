...
export default function BerhasilPage() {
  ...
  useEffect(() => {
    const colors = ["#e87a1a", "#f97316", "#fbbf24", "#e87a1a", "#ffffff"]
  ...
  return (
    <>
      ...
      {/* Background circles */}
      <div className="fixed pointer-events-none rounded-full w-[600px] h-[600px] -top-[100px] -left-[100px]" style={{ background: "radial-gradient(circle, rgba(232,122,26,0.12) 0%, transparent 70%)" }} />
      <div className="fixed pointer-events-none rounded-full w-[400px] h-[400px] -bottom-[80px] -right-[80px]" style={{ background: "radial-gradient(circle, rgba(232,122,26,0.12) 0%, transparent 70%)" }} />
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-5 py-12">
        ...
        <Link href="/" className="inline-flex items-center gap-3 mb-6 no-underline" style={{ animation: "fadeUp 0.6s 0.1s ease both" }}>
          <img src="/parstama_logo.png" alt="PARSTAMA" width={60} height={60} className="w-15 h-15 sm:w-16 sm:h-16 rounded-full object-contain" style={{ animation: "logo3D 6s ease-in-out infinite", boxShadow: "0 0 40px rgba(232,122,26,0.4)" }} />
          <span className="font-bold text-lg bg-linear-to-r from-orange-400 to-orange-600 bg-clip-text text-transparent" style={{ fontFamily: "Georgia, serif" }}>
            PARSTAMA
          </span>
        </Link>
        ...
        <div className="relative inline-flex items-center justify-center mb-8" style={{ animation: "floatY 3s ease-in-out infinite, icon3D 8s ease-in-out infinite" }}>
          <div className="absolute inset-0 rounded-full border-2 border-orange-500/50" style={{ animation: "ripple 2s ease-out infinite" }} />
          <div className="absolute inset-0 rounded-full border-2 border-orange-500/50" style={{ animation: "ripple 2s ease-out 0.6s infinite" }} />
          <div className="absolute inset-0 rounded-full border-2 border-orange-500/50" style={{ animation: "ripple 2s ease-out 1.2s infinite" }} />
          <div className="relative z-10 w-24 h-24 rounded-full bg-linear-to-br from-orange-600 to-orange-800 flex items-center justify-center text-4xl shadow-[0_0_40px_rgba(232,122,26,0.4)]" style={{ animation: "popIn 0.6s 0.2s cubic-bezier(0.175,0.885,0.32,1.275) both" }}>
            🩸
          </div>
        </div>
        ...
        <h1 className="text-[clamp(1.8rem,5vw,2.8rem)] font-extrabold mb-3 leading-tight" style={{ animation: "fadeUp 0.6s 0.5s ease both" }}>
          Pendaftaran{" "}
          <span className="bg-linear-to-r from-orange-400 via-orange-400 to-orange-400 bg-[length:200%_auto] bg-clip-text text-transparent" style={{ animation: "gradientShift 3s linear infinite" }}>
            Berhasil!
          </span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md" style={{ animation: "fadeUp 0.6s 0.95s ease both" }}>
          <Link
            href="/"
            className="flex-1 px-6 py-3.5 rounded-full text-center font-bold text-sm bg-linear-to-r from-orange-600 to-orange-800 text-white shadow-lg shadow-orange-600/30 hover:shadow-orange-600/50 hover:-translate-y-0.5 transition-all duration-200"
          >
            ← Kembali ke Beranda
          </Link>
          <Link
            href="/cek-status"
            className="flex-1 px-6 py-3.5 rounded-full text-center font-semibold text-sm border border-white/20 text-zinc-300 hover:border-orange-500 hover:text-orange-400 hover:-translate-y-0.5 transition-all duration-200"
          >
            💬 Tanya AI
          </Link>
        </div>
